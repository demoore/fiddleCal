...
class createUser
{
    ...
    public function checkUser()
    {
        ...
        $result = mysqli_query($con, "SELECT * FROM users WHERE userEmail LIKE \"$this->email\"");

        $row = mysqli_fetch_array($result);

        if ($row['userID'] != "" && $row['userEmail'] != "") {
            mysqli_close($con);
            $title = "<h1>Try Again <small>email already in use. Try a different email</small></h1>";
            include("retryCreate.php");
			echo '<font color="#f5f5f5">';
            return;
        ...
        $send = new email($this->email, "Thank you for using GD rememinder", $message);
        include("requestSent.php");
        return;
